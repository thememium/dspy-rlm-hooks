# DSPy RLM Hooks Contributing Guide

Hi! I'm really excited that you are interested in contributing to DSPy RLM Hooks. Before submitting your contribution, please make sure to take a moment and read through the following guidelines:
...
- Include relevant environment information (Python version, OS, DSPy version, etc.).
...
- [usechange](https://github.com/thememium/usechange) for release management
...
- [`uv run poe test-e2e`](#uv-run-poe-test-e2e)
...
- [`uv run poe deptry`](#uv-run-poe-deptry)
- [`uv run poe release`](#uv-run-poe-release)

### `uv run poe dev`

Run the package entrypoint in development mode:

```bash
$ uv run poe dev
```

This is equivalent to `uv run src/dspy_rlm_hooks/__init__.py`.
...
$ uv run pytest tests/test_patcher.py -v

# Run a specific test
$ uv run pytest tests/test_patcher.py::TestEnableRlmHooks -v
```
...
Create a new release using usechange:
...
- Use absolute imports from the `dspy_rlm_hooks` package for local modules
...
Thank you to all the people who have already contributed to DSPy RLM Hooks!

<a href="https://github.com/thememium/dspy-rlm-hooks/graphs/contributors"><img src="https://contrib.rocks/image?repo=thememium/dspy-rlm-hooks" /></a>
